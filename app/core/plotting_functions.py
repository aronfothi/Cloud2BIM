...
def plot_histogram(grid_full, x_values_full, y_values_full):
    plt.imshow(
        grid_full,
        origin="lower",
        extent=[x_values_full[0], x_values_full[-1], y_values_full[0], y_values_full[-1]],
        cmap="viridis",
    )
...
def plot_2d_wall_groups(wall_groups, rotated_wall_groups, rotated_wall_axes, original_wall_axes):
    ...
    for i, rotated_wall_group in enumerate(rotated_wall_groups):
        rotated_wall_group_x, rotated_wall_group_y = zip(
            *[(x, y) for x, y, _ in rotated_wall_group]
        )
        plt.plot(
            rotated_wall_group_x, rotated_wall_group_y, "--", label=f"Wall Group {i + 1} (Rotated)"
        )
        ...
        for j, original_wall_axis in enumerate(original_wall_axes):
            ...
            plt.plot(
                original_wall_axis_x, original_wall_axis_y, "r--", label=f"Original Axis {j + 1}"
            )
...
def plot_threshold_and_filtered_points(threshold, wall_group, filtered_wall_points):
    ...
    plt.scatter(
        [point[1] for point in filtered_wall_points],
        [0] * len(filtered_wall_points),
        color="g",
...
def plot_smoothed_contour(original_polygon, smoothed_polygon):
    ...
    original_polygon_inset = Polygon(
        original_polygon.get_xy(), closed=True, fill=None, edgecolor="blue"
    )
    smoothed_polygon_inset = Polygon(
        smoothed_polygon.get_xy(), closed=True, fill=None, edgecolor="red"
    )

    # Create inset of the zoomed region
    axins = inset_axes(
        ax, width="50%", height="50%", loc="center"
    )  # Adjust the size and location of the inset
...
def plot_point_cloud_data(points_xyz, n_points_array, z_array, max_n_points_array, z_step):
    ...
    h_surf_candidates = [
        (z, z + z_step) for z, n in zip(z_array, n_points_array) if n > max_n_points_array
    ]
    ...
    colors = [
        (
            "orange"
            if any(lower - z_step <= z <= upper + z_step for (lower, upper) in h_surf_candidates)
            else "blue"
        )
    ...
    ax2.scatter(
        downsampled_points[:, 1], downsampled_points[:, 2], c=colors, marker=".", alpha=1, s=0.2
    )
