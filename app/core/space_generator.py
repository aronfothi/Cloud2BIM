...
def find_disconnected_walls(walls_local):
    ...
    not_connected_walls = [
        wall
        for wall in walls_local
        if point_count[tuple(wall["start_point"])] == 1
        or point_count[tuple(wall["end_point"])] == 1
    ]
...
def divide_wall(wall, point, min_wall_length):
    ...
    if (
        new_wall_1["start_point"] == new_wall_1["end_point"]
        or new_wall_2["start_point"] == new_wall_2["end_point"]
    ):
        return None, None
    ...
    if length_new_wall_1 < min_wall_length or length_new_wall_2 < min_wall_length:
        return (
            None if length_new_wall_1 < min_wall_length else new_wall_1,
    ...
    while i < len(walls_for_processing):
        ...
        for not_connected_wall in not_connected_walls:
            ...
            if is_point_on_centerline(not_connected_wall["start_point"], wall):
                new_wall_1, new_wall_2 = divide_wall(
                    wall, not_connected_wall["start_point"], min_wall_length
                )
            ...
            if is_point_on_centerline(not_connected_wall["end_point"], wall):
                ...
                new_wall_1, new_wall_2 = divide_wall(
                    wall, not_connected_wall["end_point"], min_wall_length
                )
                if new_wall_1 and new_wall_2:
            ...
            for j, other_wall in enumerate(walls_for_processing):
                if i != j:
                    if is_point_on_centerline(wall["start_point"], other_wall):
                        new_wall_1, new_wall_2 = divide_wall(
                            other_wall, wall["start_point"], min_wall_length
                        )
                        if new_wall_1 and new_wall_2:
                        ...
                        new_wall_1, new_wall_2 = divide_wall(
                            other_wall, wall["end_point"], min_wall_length
                        )
                        if new_wall_1 and new_wall_2:
                            walls_for_processing.pop(j)  # Remove the other wall
                            walls_for_processing.insert(j, new_wall_2)  # Insert new divided walls
    ...
    for i, not_connected_wall in enumerate(not_connected_walls):
        ...
        for point_type in ["start_point", "end_point"]:
            ...
            if not point_on_centerline:
                ...
                updated_wall = extend_point_on_centerline(
                    point, not_connected_wall, walls_local, distance_threshold_extension
                )
...
def extend_point_on_centerline(
    point, not_connected_wall, walls_local, distance_threshold_extension
):
    ...
    for wall in walls_local:
        ...
        if not intersection_point:
            intersection_point = find_intersection(
                moving_point_coords, extended_point, wall_partime
            )

        if intersection_point:
            # Calculate the distance between the original point and the intersection
            distance = (
                (moving_point_coords[0] - intersection_point[0]) ** 2
                + (moving_point_coords[1] - intersection_point[1]) ** 2
            ) ** 0.5
...
def process_centerlines(walls_local, distance_threshold_extension, min_wall_length, plot=True):
    ...
    extended_walls = extend_to_centerline(
        not_connected_walls, walls_local, distance_threshold_extension
    )
    if plot:
        plot_wall_center_lines(extended_walls, "Walls extended to centerline")
    processed_walls = process_disconnected_walls(
        copy.deepcopy(walls_local), extended_walls, min_wall_length
    )
    if plot:
...
def is_point_in_polygon(point, polygon):
    ...
    return shapely_polygon.exterior.contains(shapely_point) or shapely_polygon.contains(
        shapely_point
    )
...
def identify_zones(walls_local, snapping_distance=0.5, plot_zones=True):
    ...
    updated_walls = process_centerlines(
        walls_local, snapping_distance, min_wall_length=0.02, plot=plot_zones
    )

    # Generate wall surfaces (zone_segments) and extend them with half of clipping distance
    zone_segments, zone_segments_extended = generate_space_boundaries(
        updated_walls, snapping_distance
    )
