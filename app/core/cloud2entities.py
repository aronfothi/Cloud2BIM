...
class CloudToBimProcessor:
    ...
    def _identify_walls_and_openings(self):
        ...
        from .aux_functions import identify_walls, identify_openings, split_pointcloud_to_storeys # Functions are defined in aux_functions.py
    ...
    def _generate_ifc_model(self):
        ...
        for wall in self.walls:
            ...
            wall_openings = [opening for opening in self.all_openings if opening['opening_wall_id'] == wall['wall_id']]
            ...
            wall_swept_solid_representation = self.ifc_model.wall_swept_solid_representation(start_point, end_point, wall_heights,
                                                                                        wall_thickness)
            product_definition_shape = self.ifc_model.product_definition_shape(wall_axis_representation,
            ...
            assign_object = self.ifc_model.assign_product_to_storey(wall, storeys_ifc[current_story - 1])
            ...
            for opening in wall_openings:
                ...
                opening_closed_profile = self.ifc_model.opening_closed_profile_def(float(opening_width), wall_thickness)
                opening_placement = self.ifc_model.opening_placement(start_point, wall_placement)
                opening_extrusion = self.ifc_model.opening_extrusion(opening_closed_profile, float(opening_height), start_point,
                ...
                wall_opening = self.ifc_model.create_wall_opening(opening_placement[1], opening_product_definition)
                rel_voids_element = self.ifc_model.create_rel_voids_element(wall, wall_opening)
                if opening_type == "window":
                    window_closed_profile = self.ifc_model.opening_closed_profile_def(float(opening_width), 0.01)
                    window_extrusion = self.ifc_model.opening_extrusion(window_closed_profile, float(opening_height), start_point,
                                                                end_point, float(window_sill_height), float(offset_from_start))
                    ...
                    window = self.ifc_model.create_window(opening_placement[1], window_product_definition, opening_id)
                    ...
                    door_closed_profile = self.ifc_model.opening_closed_profile_def(float(opening_width), 0.01)
                    door_extrusion = self.ifc_model.opening_extrusion(door_closed_profile, float(opening_height), start_point,
                                                                end_point, float(window_sill_height), float(offset_from_start))
                    ...
                    door = self.ifc_model.create_door(opening_placement[1], door_product_definition, opening_id)
