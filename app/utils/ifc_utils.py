...
class IFCGenerator:
    ...
    def _setup_project(self):
        ...
        # Create IFC project
        self.project = self.ifc_file.create_entity("IfcProject", 
        ...
        length_unit = self.ifc_file.create_entity("IfcSIUnit", 
            UnitType="LENGTHUNIT",
            Name="METRE"
        )
        ...
        axis_placement = self.ifc_file.create_entity("IfcAxis2Placement3D", 
            ...
            Axis=self.ifc_file.create_entity("IfcDirection", DirectionRatios=(0., 0., 1.)),
            RefDirection=self.ifc_file.create_entity("IfcDirection", DirectionRatios=(1., 0., 0.))
        )
        self.context = self.ifc_file.create_entity("IfcGeometricRepresentationContext",
            ContextType="Model",
    ...
    def create_slab(self, points: np.ndarray, thickness: float, level: float) -> Any:
        ...
        try:
            ...
            profile_coords = [(min_x, min_y), (max_x, min_y), 
                            (max_x, max_y), (min_x, max_y), (min_x, min_y)]
            points_list = [self.ifc_file.create_entity("IfcCartesianPoint", Coordinates=(x, y)) 
                          for x, y in profile_coords]
            
            polyline = self.ifc_file.create_entity("IfcPolyline", Points=points_list)
            profile = self.ifc_file.create_entity("IfcArbitraryClosedProfileDef",
                ProfileType="AREA",
                OuterCurve=polyline
            )
            
            # Create extrusion
            direction = self.ifc_file.create_entity("IfcDirection", DirectionRatios=(0., 0., 1.))
            position = self.ifc_file.create_entity("IfcAxis2Placement3D",
                Location=self.ifc_file.create_entity("IfcCartesianPoint", Coordinates=(0., 0., level)),
                Axis=direction
            )
            
            solid = self.ifc_file.create_entity("IfcExtrudedAreaSolid",
                SweptArea=profile,
                Position=position,
            ...
            shape = self.ifc_file.create_entity("IfcShapeRepresentation",
            ...
            # Create slab entity
            slab = self.ifc_file.create_entity("IfcSlab",
                GlobalId=ifcopenshell.guid.new(),
            ...
            # Add to building
            self.ifc_file.create_entity("IfcRelContainedInSpatialStructure",
                GlobalId=ifcopenshell.guid.new(),
    ...
    def create_wall(self, points: np.ndarray, thickness: float, height: float, 
                   base_level: float) -> Any:
        """
        ...
        try:
            ...
            axis_points = [
                self.ifc_file.create_entity("IfcCartesianPoint", 
            ...
            position = self.ifc_file.create_entity("IfcAxis2Placement3D",
                Location=self.ifc_file.create_entity("IfcCartesianPoint", 
                    Coordinates=(0., 0., base_level))
            )
            
            material_layer = self.ifc_file.create_entity("IfcMaterialLayer",
                Material=self.ifc_file.create_entity("IfcMaterial", 
                    Name=self.settings.get("material_for_walls", "Concrete")),
                LayerThickness=thickness
            )
            
            material_layer_set = self.ifc_file.create_entity("IfcMaterialLayerSet",
                MaterialLayers=[material_layer]
            )
            
            wall = self.ifc_file.create_entity("IfcWallStandardCase",
                GlobalId=ifcopenshell.guid.new(),
            ...
            self.ifc_file.create_entity("IfcRelAssociatesMaterial",
                GlobalId=ifcopenshell.guid.new(),
                RelatedObjects=[wall],
