...
def simple_room_point_cloud():
    ...
    left_wall_points = np.column_stack(
        [left_wall_points[:, 0], np.zeros(points_per_surface), left_wall_points[:, 1]]
    )
    ...
    right_wall_points = np.column_stack(
        [right_wall_points[:, 0], np.full(points_per_surface, length), right_wall_points[:, 1]]
    )
...
def processor_with_simple_room(tmp_path, simple_room_point_cloud):
    ...
    config = {
        "input": {"point_cloud": {"file_path": str(pcd_path)}},
        "processing": {
            "voxel_size": 0.05,
            "min_points": 100,
            "wall_thickness": 0.2,
