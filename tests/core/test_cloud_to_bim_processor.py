...
def test_cloud_to_bim_processor_full_run(job_setup):
    ...
    assert expected_mapping_path.exists(), f"Point mapping file {expected_mapping_path} was not created."
    assert expected_mapping_path.stat().st_size > 0, f"Point mapping file {expected_mapping_path} is empty."

    try:
        with open(expected_mapping_path, 'r') as f:
            mapping_data = json.load(f)
        assert isinstance(mapping_data, dict), "Point mapping file does not contain valid JSON dictionary."
